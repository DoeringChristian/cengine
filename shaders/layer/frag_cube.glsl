...
void main (void){
    ...
    if(frag_uv.x > 0.0 && frag_uv.x < 0.25 && frag_uv.y < 0.5 && frag_uv.y > 0.25){
        o_color = vec4(texture(u_texture, vec3(frag_uv.x, frag_uv.y, -1)).r, 0, 0, 1);
    }
    else if(frag_uv.x > 0.25 && frag_uv.x < 0.5 && frag_uv.y < 0.5 && frag_uv.y > 0.25){
        o_color = vec4(texture(u_texture, vec3(1, frag_uv.x, frag_uv.y)).r, 0, 0, 1);
    }
    else if(frag_uv.x > 0.5 && frag_uv.x < 0.75 && frag_uv.y < 0.5 && frag_uv.y > 0.25){
        o_color = vec4(texture(u_texture, vec3(frag_uv.x, frag_uv.y, 1)).r, 0, 0, 1);
    }
    else if(frag_uv.x > 0.75 && frag_uv.x < 0.1 && frag_uv.y < 0.5 && frag_uv.y > 0.25){
        o_color = vec4(texture(u_texture, vec3(-1, frag_uv.x, frag_uv.y)).r, 0, 0, 1);
    }
    else if(frag_uv.x > 0.5 && frag_uv.x < 0.75 && frag_uv.y < 0.25 && frag_uv.y > 0.0){
        o_color = vec4(texture(u_texture, vec3(frag_uv.x, -1, frag_uv.y)).r, 0, 0, 1);
    }
